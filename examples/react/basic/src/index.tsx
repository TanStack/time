...
function parseEventDuration(event: Event<Resource>): number {
...
function ScheduleView({
  ...
  const getEventPosition = (event: Event<Resource>) => {
    const { hour, minute } = parseEventTime(event)
    const duration = parseEventDuration(event)
    const top = (hour * 60 + minute) * (slotHeight / 60)
