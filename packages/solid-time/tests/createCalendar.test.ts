  ...
  test('the resize surface exists only when resize is composed', () =>
    createRoot((dispose) => {
      ...
      const withResize = createCalendar({
        features: calendarFeatures([eventRecurrenceFeature, eventResizeFeature]),
        viewMode: { value: 1, unit: 'week' },
        timeZone: 'UTC',
        events,
      })
      ...
      expect(
        withResize.getResizeHandleProps('e1', 'bottom', `${DAY}T09:00:00`, `${DAY}T10:00:00`),
      ).toBe(withResize.getResizeHandleProps('e1', 'bottom', `${DAY}T09:00:00`, `${DAY}T10:00:00`))
