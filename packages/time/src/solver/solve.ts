...
function toMs(
  ...
  return {
    startMs: Temporal.PlainDateTime.from(span.start).toZonedDateTime(timeZone)
      .epochMilliseconds,
    endMs: Temporal.PlainDateTime.from(span.end).toZonedDateTime(timeZone)
      .epochMilliseconds,
  };
}
...
export function solve(request: SolveRequest): SolveResult {
  ...
  if (stalled) {
    conflicts.push({
      ...
      eventIds: [
        ...new Set(
          relaxable.flatMap((dep) => [dep.predecessorId, dep.successorId]),
        ),
      ],
