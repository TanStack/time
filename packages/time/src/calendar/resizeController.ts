...
export class ResizeController<TResource extends Resource, TEvent extends Event<TResource>> {
  ...
  constructor(host: ResizeHost<TResource, TEvent>, options: ResizeControllerOptions = {}) {
    this._host = host
  ...
  handleMouseUp() {
    ...
    if (currentState.eventId && currentState.previewStart && currentState.previewEnd) {
      const hasChanged =
        !original ||
  ...
  private _processMouseMove(e: MouseEvent) {
    ...
    if (orientation === 'horizontal') {
      ...
      if (targetDayDate !== originalDayDate) {
        ...
        const dayDiff = Math.round((targetDate.getTime() - originalDate.getTime()) / MS_PER_DAY)
