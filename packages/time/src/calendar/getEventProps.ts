...
const toZonedDateTime = (
  ...
  Temporal.PlainDateTime.from(toPlainDateTimeString(dateInput)).toZonedDateTime(timeZone)
...
const dayKeyOf = (value: EventDateTimeInput): string => toPlainDateTimeString(value).slice(0, 10)

const sameSegment = (a: Event, b: Event): boolean =>
  a.id === b.id && toPlainDateTimeString(a.start) === toPlainDateTimeString(b.start)
...
export const getEventProps = (
  ...
  const isTimeGridView = state.viewMode.unit === 'week' || state.viewMode.unit === 'day'

  if (!isTimeGridView) {
    return baseProps
  }

  const daySegments = (options.daySegments ?? eventMap.get(dayKeyOf(event.start)) ?? []).filter(
    (e) => Boolean(e.allDay) === Boolean(event.allDay),
  )
