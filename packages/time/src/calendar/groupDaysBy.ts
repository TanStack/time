...
const plainDateOf = (isoDate: string): Temporal.PlainDate => Temporal.PlainDate.from(isoDate)
...
const filler = <TResource extends Resource, TEvent extends Event<TResource>>(
  ...
  return {
    ...
    dayOfMonth: toCalendarDate(isoDate, calendar).day,
    events: [],
    allDayEvents: [],
    isToday: false,
...
export type GroupDaysByProps<
  TResource extends Resource,
  TEvent extends Event<TResource> = Event<TResource>,
> = GroupDaysByMonthProps<TResource, TEvent> | GroupDaysByWeekProps<TResource, TEvent>
...
>({
  ...
  locale,
  calendar = 'iso8601',
}: GroupDaysByProps<TResource, TEvent>): Array<Array<Day<TResource, TEvent> | null>> => {
  ...
  switch (unit) {
    case 'week': {
        ...
        if (currentWeek.length === 0 && dayDate?.dayOfWeek !== weekStartsOn) {
          if (dayDate) {
            ...
            for (let i = 0; i < dayOfWeek; i++) {
              ...
              currentWeek.push(
                fillMissingDays ? filler<TResource, TEvent>(newDate, calendar) : null,
              )
            }
          }
        }
      ...
      if (currentWeek.length > 0) {
        while (currentWeek.length < 7) {
          ...
          currentWeek.push(fillMissingDays ? filler<TResource, TEvent>(newDate, calendar) : null)
        }
        weeks.push(currentWeek)
      }
    ...
    case 'workWeek': {
      ...
      days.forEach((day) => {
        const dayDate = day ? plainDateOf(day.isoDate) : null
        if (currentWorkWeek.length === 0 && dayDate?.dayOfWeek !== weekStartsOn) {
          if (dayDate) {
            const dayOfWeek = (dayDate.dayOfWeek - weekStartsOn + 7) % 7
            for (let i = 0; i < dayOfWeek; i++) {
              ...
              if (!weekend.includes(newDay.dayOfWeek)) {
                currentWorkWeek.push(
                  fillMissingDays ? filler<TResource, TEvent>(newDay, calendar) : null,
                )
              }
            }
          }
      ...
      if (currentWorkWeek.length > 0) {
        while (currentWorkWeek.length < 5) {
          const lastIsoDate = currentWorkWeek[currentWorkWeek.length - 1]?.isoDate
          const lastDate = lastIsoDate
            ? plainDateOf(lastIsoDate)
            : Temporal.PlainDate.from('2024-01-01')
          ...
          if (!weekend.includes(nextDate.dayOfWeek)) {
            currentWorkWeek.push(
              fillMissingDays ? filler<TResource, TEvent>(nextDate, calendar) : null,
            )
          }
        }
        workWeeks.push(currentWorkWeek)
