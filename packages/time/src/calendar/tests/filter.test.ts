...
const titlesOn = (cal: {
  getEventsByDate: (date: string) => Array<TestEvent>;
}) =>
  cal
    .getEventsByDate(DAY)
...
describe("eventFilterFeature", () => {
  ...
  test("does not remove filtered events from the store", () => {
    ...
    expect(cal.getHiddenEvents().map((event) => event.id)).toEqual([
      "personal",
    ]);
  ...
  test("filters expanded recurrence occurrences", () => {
    const cal = createTestCalendar(
      calendarFeatures([eventFilterFeature, eventRecurrenceFeature]),
      [
        {
          ...workEvent,
  ...
  test("hidden events do not take layout space from visible ones", () => {
    const cal = createTestCalendar(
      calendarFeatures([eventFilterFeature, dayEventLayoutFeature]),
      [workEvent, personalEvent],
    );

    expect(
      cal.getEventProps(cal.getEventsByDate(DAY)[0]!).layout?.concurrency,
    ).toBe(2);
