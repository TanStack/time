...
describe('CalendarCore', () => {
  ...
  describe('getUnavailabilityDetails', () => {
    ...
    test('resolves project, resource and event calendars together', () => {
      ...
      expect(cal.getUnavailableMinuteRanges(DATE_MON, { resourceIds: ['ana'] })).toEqual([
        { startMinutes: 0, endMinutes: 480 },
        { startMinutes: 1020, endMinutes: 1440 },
      ])

      expect(cal.getUnavailableMinuteRanges(DATE_TUE, { resourceIds: ['ana'] })).toEqual([
      ...
      expect(cal.validateEventPlacement({ ...lateEvent, calendarId: 'after-hours' }).blocked).toBe(
        false,
      )
    })
  ...
  describe('validateMove', () => {
    test('returns blocked:false for unknown event id', () => {
      ...
      const r = cal.validateMove('nonexistent', `${DATE_MON}T14:00:00`, `${DATE_MON}T15:00:00`)
    ...
    test('allows move fully inside availability', () => {
      ...
      const r = cal.validateMove('e1', `${DATE_MON}T10:00:00`, `${DATE_MON}T11:00:00`)
      expect(r.blocked).toBe(false)
    })
    ...
    test('blocks when the moved range overlaps unavailable hours', () => {
      ...
      const r = cal.validateMove('e1', `${DATE_MON}T07:00:00`, `${DATE_MON}T08:00:00`)
    ...
    test('allows move when event has no resources (no availability to violate)', () => {
      ...
      const r = cal.validateMove('e1', `${DATE_MON}T06:00:00`, `${DATE_MON}T07:00:00`)
      expect(r.blocked).toBe(false)
    })
    ...
    test('blocks when extending end pushes a dependent into unavailable time', () => {
      ...
      const r = cal.validateMove('p', `${DATE_MON}T09:00:00`, `${DATE_MON}T17:00:00`)
      expect(r.blocked).toBe(true)
    })
    ...
    test('blocks transitive dependent when cascade would violate availability', () => {
      ...
      const r = cal.validateMove('a', `${DATE_MON}T09:00:00`, `${DATE_MON}T15:00:00`)
      expect(r.blocked).toBe(true)
    })
    ...
    test('does not run downstream availability check when new end is not extended', () => {
      ...
      const r = cal.validateMove('a', `${DATE_MON}T09:00:00`, `${DATE_MON}T10:00:00`)
      expect(r.blocked).toBe(false)
    })
    ...
    test('treats split multi-day segment rows as non-targets (no _originalStart match)', () => {
      ...
      const r = cal.validateMove('e1', `${DATE_TUE}T10:00:00`, `${DATE_TUE}T11:00:00`)
      expect(r.blocked).toBe(false)
    })
  })
  ...
  describe('capacity / consumption', () => {
    ...
    describe('validateMove with capacity', () => {
      test('blocks move when destination overlap exceeds capacity', () => {
        ...
        const r = cal.validateMove('e2', `${DATE_MON}T09:00:00`, `${DATE_MON}T10:00:00`)
      ...
      test('allows move when self consumption would still fit', () => {
        ...
        const r = cal.validateMove('e2', `${DATE_MON}T14:00:00`, `${DATE_MON}T15:00:00`)
  ...
  describe('dependency types (FS, SS, FF, SF)', () => {
    ...
    describe('validateMove - predecessor cascade per type', () => {
      test('FS: moving successor earlier pulls predecessor back into unavailable hours → blocked', () => {
        ...
        const r = cal.validateMove('s', `${DATE_MON}T08:00:00`, `${DATE_MON}T09:00:00`)
        expect(r.blocked).toBe(true)
      ...
      test('SS: moving successor before predecessor.start pulls predecessor back', () => {
        ...
        const r = cal.validateMove('s', `${DATE_MON}T07:30:00`, `${DATE_MON}T08:30:00`)
        expect(r.blocked).toBe(true)
        expect(r.blockedEventTitle).toBe('P')
      })
      ...
      test('FF: shrinking successor.end below predecessor.end pulls predecessor back', () => {
        ...
        const allowed = cal.validateMove('s', `${DATE_MON}T08:00:00`, `${DATE_MON}T09:00:00`)
        expect(allowed.blocked).toBe(false)

        const blocked = cal.validateMove('s', `${DATE_MON}T07:30:00`, `${DATE_MON}T08:30:00`)
        expect(blocked.blocked).toBe(true)
      ...
      test('SF: shrinking successor.end below predecessor.start pulls predecessor back', () => {
        ...
        const r = cal.validateMove('s', `${DATE_MON}T06:30:00`, `${DATE_MON}T07:30:00`)
        expect(r.blocked).toBe(true)
        expect(r.blockedEventTitle).toBe('P')
      })
      ...
      test('FS: moving successor later (constraint already satisfied) does not touch predecessor', () => {
        ...
        const r = cal.validateMove('s', `${DATE_MON}T14:00:00`, `${DATE_MON}T15:00:00`)
        expect(r.blocked).toBe(false)
      })
    })
    ...
    describe('createDependency', () => {
      ...
      test('a lagged link cascades by the lag when the predecessor moves', () => {
        ...
        expect(
          cal.getAffectedByDelta('p', 60 * 60 * 1000).map((shift) => ({
            id: shift.event.id,
    ...
    describe('manuallyScheduled', () => {
      ...
      test('an anchored dependent stays put and blocks the move', async () => {
        ...
        expect(events.find((e) => e.id === 'p')!.start).toBe(`${DATE_MON}T10:00:00`)
        expect(events.find((e) => e.id === 's')!.start).toBe(`${DATE_MON}T11:00:00`)
      })
      ...
      test('a move the anchor still has slack for is allowed', async () => {
        ...
        assert(result.success)
        expect(cal.getEvents().find((e) => e.id === 's')!.start).toBe(`${DATE_MON}T11:00:00`)
      })

      test('validateMove reports the anchor that cannot follow', () => {
        expect(
          anchoredPair().validateMove('p', `${DATE_MON}T11:00:00`, `${DATE_MON}T12:00:00`),
      ...
      test('the anchor itself can still be moved, and pushes its dependents', () => {
        ...
        expect(cal.getEvents().find((e) => e.id === 's')!.start).toBe(`${DATE_MON}T12:30:00`)
      })
    ...
    describe('scheduling constraints', () => {
      ...
      test('checkEventConstraint answers for a proposed span', () => {
        ...
        expect(
          cal.checkEventConstraint(event, `${DATE_MON}T12:00:00`, `${DATE_MON}T13:00:00`),
    ...
    describe('effort and duration', () => {
      ...
      test('answers the working duration of a proposed span', () => {
        ...
        expect(cal.getWorkingDuration(event, `${DATE_MON}T09:00:00`, `${DATE_MON}T10:30:00`)).toBe(
          90,
        )
      })
      ...
      test('validateMove reports the mismatch', () => {
        expect(
          withDuration(240).validateMove('a', `${DATE_MON}T09:00:00`, `${DATE_MON}T12:00:00`),
        ).toMatchObject({
          blocked: true,
          blockedEventTitle: 'A',
    ...
    describe('fetchEventsForRange', () => {
      test('resolves immediately and is a no-op when fetchEvents is not configured', async () => {
        const cal = createTestCalendar()
        await expect(cal.fetchEventsForRange(DATE_MON, DATE_TUE)).resolves.toBeUndefined()
      ...
      test('emits events:set when new events are merged in', async () => {
        ...
        const setCall = emitSpy.mock.calls.find(([name]) => name === 'events:set')
    ...
    describe('event emissions', () => {
      test('commitAdd emits event:added', () => {
        ...
        const addCall = emitSpy.mock.calls.find(([name]) => name === 'event:added')
      ...
      test('commitUpdate emits event:updated with the patch', () => {
        ...
        const upd = emitSpy.mock.calls.find(([name]) => name === 'event:updated')
      ...
      test('removeEvent emits event:removed', () => {
        ...
        const rem = emitSpy.mock.calls.find(([name]) => name === 'event:removed')
    ...
    describe('getTimelineLayout', () => {
      test('returns one row per resource', () => {
        ...
        expect(layout.rows.map((r) => r.resource.id)).toEqual(['r1', 'r2', 'r3'])
      })
      ...
      test('exposes fractions consistent with the percentage positions', () => {
        ...
        expect(item.width).toBeCloseTo((item.endFraction - item.startFraction) * 100)
    ...
    describe('getEventProps', () => {
      ...
      test('lays all-day events out against each other, not against timed ones', () => {
        ...
        const props = cal.getEventProps(cal.getEvents().find((e) => e.id === 'holiday')!) as {
          layout: { concurrency: number }
          style: { width: string }
        }
      ...
      test('keeps an event outside the busy cluster at full width', () => {
        ...
        const props = cal.getEventProps(cal.getEvents().find((e) => e.id === 'alone')!) as {
          style: { left: string; width: string }
        }
      ...
      test('detects overlap with a sibling event', () => {
        ...
        const props = cal.getEventProps(cal.getEvents().find((e) => e.id === 'e1')!)
      ...
      test('top-edge resize keeps the bottom (end) fixed at the real end', () => {
        ...
        for (const previewStart of [`${day}T08:57:00`, `${day}T08:45:00`, `${day}T08:00:00`]) {
    ...
    describe('validateMove backward cascade - allowed when target available', () => {
      test('FS: allows move when predecessor pull-back stays within availability', () => {
        ...
        const r = cal.validateMove('s', `${DATE_MON}T10:30:00`, `${DATE_MON}T11:30:00`)
        expect(r.blocked).toBe(false)
      })
    })
    ...
    describe('createDependency - shape and idempotency edges', () => {
      test('emits a single event:updated when dependency is added without reschedule', () => {
        ...
        const targetUpdates = emitSpy.mock.calls.filter(
          ([name, payload]) => name === 'event:updated' && payload.eventId === 's',
        )
  ...
  describe('undo/redo', () => {
    ...
    describe('command diffs', () => {
      ...
      test('one undo reverses an update and everything it cascaded', () => {
        ...
        cal.commitUpdate('a', { end: `${DATE_MON}T12:30:00` })
        expect(cal.getEvents().find((e) => e.id === 'b')!.start).toBe(`${DATE_MON}T12:30:00`)
      ...
      test("keeps createDependency's reschedule in a single entry", () => {
        ...
        expect(cal.getEvents().find((e) => e.id === 'b')!.start).toBe(`${DATE_MON}T13:00:00`)
      ...
      test('reports the events an undo touched', () => {
        ...
        expect(payload.updated.map((e: { eventId: string }) => e.eventId)).toEqual(['b', 'a'])
      })
  ...
  describe('recurrence exceptions', () => {
    ...
    test('expands recurring master occurrence and later occurrences', () => {
      const cal = createTestCalendar({ events: [recurringEvent] })
      expect(cal.getEventsByDate('2025-06-02').map((e) => e.id)).toEqual(['rec-ex'])
      expect(cal.getEventsByDate('2025-06-09').map((e) => e.id)).toEqual(['rec-ex_1'])
    })
    ...
    test('override and EXDATE can target original master occurrence', () => {
      ...
      expect(movedCal.getEventsByDate('2025-06-02')[0]!.start).toBe('2025-06-02T15:00:00')
    ...
    test('editRecurringEvent with scope this creates an override', async () => {
      ...
      expect(cal.getEventsByDate('2025-06-16')[0]!.start).toBe('2025-06-16T09:00:00')
    })
    ...
    test('removeRecurringEvent with scope this creates EXDATE', () => {
      ...
      expect(cal.getEvents()[0]!.recurrence?.exDates).toEqual(['2025-06-09T09:00:00'])
      expect(cal.getEventsByDate('2025-06-09')).toHaveLength(0)
      expect(cal.getEventsByDate('2025-06-16')).toHaveLength(1)
    })
    ...
    test('validateResize checks capacity for a recurring occurrence', () => {
      ...
      expect(result.blocked).toBe(true)
      expect(result.error?.conflicts?.[0]?.resourceDetails[0]?.reason).toBe('capacity')
    })
    ...
    test('editRecurringEvent with scope thisAndFollowing splits the series', async () => {
      ...
      expect(cal.getEventsByDate('2025-06-09')[0]!.start).toBe('2025-06-09T09:00:00')
      expect(cal.getEventsByDate('2025-06-16')[0]!.start).toBe('2025-06-16T15:00:00')
    })
    ...
    test('editRecurringEvent rejects an occurrenceStart outside the series', async () => {
      ...
      assert(!result.success)
      expect(result.error.message).toBe('Occurrence "2025-06-10T09:00:00" not found.')
    ...
    test('editRecurringEvent with scope thisAndFollowing at the master start keeps later overrides', async () => {
      ...
      expect(cal.getEventsByDate('2025-06-09')[0]!.title).toBe('Moved Later One')
    })
  ...
  describe('isPending', () => {
    ...
    test('clears when a fetch rejects while another is in flight', async () => {
      ...
      await second
      expect(cal.store.state.isPending).toBe(false)
      expect(cal.getLoadedRanges().some((r) => r.start === '2024-03-18')).toBe(false)
    })
  })
})
