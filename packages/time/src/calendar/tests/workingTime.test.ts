...
describe("workingTimeFeature", () => {
  it("names the calendar that governs a target", () => {
    ...
    expect(cal.getEffectiveCalendar({ calendarId: "late-exception" })).toBe(
      "late-exception",
    );
  });
  ...
  it("backs the availability feature's shading read", () => {
    ...
    expect(
      composed.getUnavailableMinuteRanges(MONDAY, { resourceIds: ["r2"] }),
    ).toEqual(composed.getNonWorkingMinutes(MONDAY, { resourceIds: ["r2"] }));
  });
