...
interface PeerReaderApi<TResource extends Resource, TEvent extends Event<TResource>> {
  masterTitleOf: (eventId: string) => TEvent['title'] | undefined
}

declare module '../features' {
  interface FeatureApiRegistry<TResource extends Resource, TEvent extends Event<TResource>> {
...
describe('calendarFeatures composition', () => {
  ...
  test("a feature's kernel module is mounted with it", async () => {
    const cal = createTestCalendar(calendarFeatures([eventRecurrenceFeature]), [recurringEvent])
    cal.goToSpecificPeriod(DAY)
  ...
  test('occurrence edits are unreachable without the recurrence feature', () => {
    const cal = createTestCalendar(calendarFeatures([dayEventLayoutFeature]), [recurringEvent])

    expect(() => uncomposed(cal).editRecurringEvent('rec_1', {}, { scope: 'this' })).toThrow(
      'CalendarCore: "editRecurringEvent" requires eventRecurrenceFeature. Compose it via calendarFeatures([eventRecurrenceFeature, ...]).',
    )
    expect(() => uncomposed(cal).removeRecurringEvent('rec_1', { scope: 'all' })).toThrow(
  ...
  test("throws when a composed feature's required peer is missing", () => {
    expect(() => createTestCalendar(calendarFeatures([eventResizeFeature]))).toThrow(
      /"resize" requires "recurrence"/,
    )

    expect(() =>
      createTestCalendar(calendarFeatures([eventResizeFeature, eventRecurrenceFeature])),
    ).not.toThrow()
  })
  ...
  test('throws when two features contribute the same api key', () => {
    ...
    expect(() =>
      createTestCalendar(calendarFeatures([dayEventLayoutFeature, shadowFeature])),
  ...
  test('occurrences are read back only when recurrence is composed', () => {
    ...
    const withRecurrence = createTestCalendar(calendarFeatures([eventRecurrenceFeature]), [
      recurringEvent,
    ])
    withRecurrence.goToSpecificPeriod(nextWeek)
    expect(withRecurrence.getEventsByDate(nextWeek)).toHaveLength(1)

    const withoutRecurrence = createTestCalendar(calendarFeatures([dayEventLayoutFeature]), [
      recurringEvent,
    ])
  ...
  test('a cascade that violates availability blocks the whole batch', () => {
    ...
    const blocked = createTestCalendar(
      calendarFeatures([workingTimeFeature, resourceAvailabilityFeature, eventDependencyFeature]),
  ...
  test('validateMove stops consulting availability when it is absent', () => {
    ...
    expect(
      withAvailability.validateMove('late', `${DAY}T20:00:00`, `${DAY}T21:00:00`).blocked,
    ).toBe(true)
    ...
    expect(
      withoutAvailability.validateMove('late', `${DAY}T20:00:00`, `${DAY}T21:00:00`).blocked,
    ).toBe(false)
  })

  test('availability reads are unreachable without the feature', () => {
    const cal = createTestCalendar(calendarFeatures([dayEventLayoutFeature]), [], [morningRoom])
  ...
  test("a feature reads a declared peer's api", () => {
    ...
    const cal = createTestCalendar(calendarFeatures([eventRecurrenceFeature, peerReadingFeature]), [
      recurringEvent,
    ])

    expect(cal.masterTitleOf('rec')).toBe('Recurring')
    expect(() => createTestCalendar(calendarFeatures([peerReadingFeature]))).toThrow(
      /"peerReader" requires "recurrence"/,
    )
  })
  ...
  test('resize blocks on unavailable time only with availability composed', () => {
    ...
    const events = [{ ...eveningEvent, start: `${DAY}T09:00:00`, end: `${DAY}T10:00:00` }]
    ...
    expect(withoutAvailability.validateResize(resizeOptions).blocked).toBe(false)
  })
  ...
  test("resize across days blocks on the target day's unavailability", () => {
    ...
    expect(validation.error?.message).toMatch(/^Unavailable: Event at 23:00 conflicts with /)
  })
  ...
  test('throws when a feature shadows a core method', () => {
    ...
    expect(() => createTestCalendar(calendarFeatures([coreShadowFeature]))).toThrow(
      /contributes api "getEvents", which shadows a core method/,
    )
  })
})
