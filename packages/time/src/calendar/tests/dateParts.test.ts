...
function createTestCalendar(
  overrides: Partial<Parameters<typeof createCalendar<typeof stockFeatures>>[0]> = {},
) {
...
describe('calendar date parts', () => {
  ...
  test('formats a period with defaults, overrides and units', () => {
    ...
    expect(calendar.formatPeriod(ISO_DATE, { unit: 'day' })).toBe('July 25, 2025')
    expect(calendar.formatPeriod(ISO_DATE, { unit: 'year' })).toBe('2025')
    expect(calendar.formatPeriod(ISO_DATE, { locale: 'de-DE' })).toBe('Juli 2025')
    expect(calendar.formatPeriod(ISO_DATE, { calendar: 'chinese' })).not.toBe('July 2025')
  })
})
