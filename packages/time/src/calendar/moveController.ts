...
export class MoveController<TResource extends Resource, TEvent extends Event<TResource>> {
  ...
  constructor(host: MoveHost<TResource, TEvent>, options: MoveControllerOptions = {}) {
    this._host = host
  ...
  moveTo(args: MoveToArgs) {
    ...
    const minuteShift = original.granularity === 'day' ? 0 : this._toMinutes(args)

    const snapped = snapToInterval(minuteShift, constraints?.snapToMinutes ?? 15)
