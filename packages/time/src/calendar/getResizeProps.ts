...
const extractDateFromDateTime = (dateTime: string): string => dateTime.split('T')[0] ?? dateTime
...
const getMinutesFromMidnight = (zdt: Temporal.ZonedDateTime): number => zdt.hour * 60 + zdt.minute
...
export function calculateResizedEvent(options: CalculateResizedEventOptions): ResizedEventResult {
  const { originalStart, originalEnd, edge, deltaMinutes, timeZone, constraints = {} } = options
  ...
  const startZdt = Temporal.PlainDateTime.from(originalStart).toZonedDateTime(timeZone)
  const endZdt = Temporal.PlainDateTime.from(originalEnd).toZonedDateTime(timeZone)
  ...
  if (effectiveEdge === 'top') {
    ...
    if (isEntireDayUnavailable) {
      ...
      for (const range of unavailableRanges) {
        if (newStartMinutes >= range.startMinutes && newStartMinutes < range.endMinutes) {
    ...
    if (isEntireDayUnavailable) {
      ...
      for (const range of unavailableRanges) {
        if (newEndMinutes > range.startMinutes && newEndMinutes <= range.endMinutes) {
...
export function getSegmentInfo(event: {
  ...
  const isSplitEvent = event._originalStart !== undefined || event._originalEnd !== undefined
...
export function calculateSegmentResizePreview(options: ResizePreviewOptions): SegmentResizePreview {
  const { dayDate, originalStart, originalEnd, previewStart, previewEnd } = options

  const previewStartDate = extractDateFromDateTime(previewStart)
  const previewEndDate = extractDateFromDateTime(previewEnd)

  const previewAffectsThisDay = dayDate >= previewStartDate && dayDate <= previewEndDate

  const hasChanged = previewStart !== originalStart || previewEnd !== originalEnd
...
export function calculateGhostPreviewStyle(options: GhostPreviewOptions): PositionStyle | null {
  const { dayDate, previewStart, previewEnd } = options

  const previewStartDate = extractDateFromDateTime(previewStart)
  const previewEndDate = extractDateFromDateTime(previewEnd)

  const isDayInPreviewRange = dayDate >= previewStartDate && dayDate <= previewEndDate
...
export function calculateTimelineResizePreview(
  ...
  const startDayOffset = Math.round(
    (new Date(startDateStr).getTime() - new Date(firstDayIso).getTime()) / (1000 * 60 * 60 * 24),
  )
  const endDayOffset = Math.round(
    (new Date(endDateStr).getTime() - new Date(firstDayIso).getTime()) / (1000 * 60 * 60 * 24),
  )

  const startHours = startDayOffset * 24 + (startTimeParts[0] ?? 0) + (startTimeParts[1] ?? 0) / 60
  const endHours = endDayOffset * 24 + (endTimeParts[0] ?? 0) + (endTimeParts[1] ?? 0) / 60
...
export function getEventDisplayTimeRange(options: {
  ...
  const { originalStart, originalEnd, isBeingResized, previewStart, previewEnd } = options

  const displayStart = isBeingResized && previewStart ? previewStart : originalStart
