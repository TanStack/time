...
export interface EventFilterApi<
  TResource extends Resource,
  TEvent extends Event<TResource>,
> {
  setEventFilter: (
    id: string,
    predicate: EventFilterPredicate<TResource, TEvent> | null,
  ) => void;
...
>(): CalendarFeature<
  ...
  TEvent,
  object,
  EventFilterApi<TResource, TEvent>,
  "filter"
> {
  ...
  return {
    ...
    api: (host) => ({
      ...
      isEventVisible,
      getHiddenEvents: () =>
        host.getEvents().filter((event) => !isEventVisible(event)),
    }),
  };
}
