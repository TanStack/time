...
import type { Day, Event, Resource, TimelineLayout, TimelineResourceRow } from '../types'
import type { CalendarFeature, CalendarHost } from './types'

export interface TimelineApi<TResource extends Resource, TEvent extends Event<TResource>> {
...
>(): CalendarFeature<TResource, TEvent, object, TimelineApi<TResource, TEvent>, 'timeline'> {
  ...
  return {
    name: 'timeline',
    api: (host) => ({
      getEventsByResource: () => mergedByResource(host, host.getDaysWithEvents()),
      getTimelineLayout: () => {
        ...
        const rows: Array<TimelineResourceRow<TResource, TEvent>> = (options.resources ?? []).map(
          (resource) => {
