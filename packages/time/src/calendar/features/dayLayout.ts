...
export interface DayLayoutApi<TResource extends Resource, TEvent extends Event<TResource>> {
...
>(): CalendarFeature<TResource, TEvent, object, DayLayoutApi<TResource, TEvent>, 'dayLayout'> {
  const daySegments = (host: CalendarHost<TResource, TEvent>, event: TEvent): Array<TEvent> => {
    ...
    return host.getEventMap({ start: isoDate, end: nextDay }).get(isoDate) ?? []
  }
