...
export interface MoveFeatureApi<
  ...
  return {
    ...
    api: (host, _module, peers) => {
      const findEvent = (eventId: string): TEvent | undefined =>
        host.getEvent(eventId) ??
        host.getEvents().find((event) => event.id === eventId);
      ...
      const validateEventMove = (
        ...
        const event = findEvent(eventId);
        const masterId = event
          ? peers.recurrence.getMasterEvent(event).id
          : eventId;
        ...
        const conflict = event
          ? peers.availability?.checkEventAvailability(
              event,
              result.start,
              result.end,
            )
