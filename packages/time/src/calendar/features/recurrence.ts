...
import { editOccurrenceIntent, recurrenceModule, removeOccurrenceIntent } from '~/kernel/modules'
import {
...
export interface RecurrenceEditApi<TResource extends Resource, TEvent extends Event<TResource>> {
  editRecurringEvent: (
    eventId: string,
...
export interface RecurrenceReadApi<TResource extends Resource, TEvent extends Event<TResource>> {
...
> {
  const resolveMaster = (
    ...
    eventId: string,
  ): TEvent | undefined => host.getEvent(eventId) ?? host.getEvent(masterIdOf(eventId))

  const cursor = (host: CalendarHost<TResource, TEvent>, fromDate?: EventDateTimeInput): string =>
    fromDate ? toPlainDateTimeString(fromDate).slice(0, 10) : host.getState().activeDate
  ...
  return {
    ...
    api: (host, module) => ({
      getMasterEvent: (event) => module.getMasterEvent(event as TEvent & KernelEvent) as TEvent,
      resolveOccurrence: (eventId, occurrenceStart, fallbackStart) => {
        ...
        const resolvedStart =
          occurrenceStart ?? (direct ? toPlainDateTimeString(direct.start) : fallbackStart)
        if (!resolvedStart) return direct

        return getRecurringOccurrence<TResource, TEvent>(master, resolvedStart) ?? direct ?? master
      },
      goToNextOccurrence: (eventId, fromDate) => {
        const master = resolveMaster(host, eventId)
        if (!master) return

        const next = nextOccurrenceDate<TResource, TEvent>(master, cursor(host, fromDate))
      ...
      goToPreviousOccurrence: (eventId, fromDate) => {
        ...
        const previous = previousOccurrenceDate<TResource, TEvent>(master, cursor(host, fromDate))
      ...
      editRecurringEvent: async (eventId, updates, options) => {
        ...
        const occurrenceStart = resolveOccurrenceStart(master.start, options.occurrenceStart)
        const occurrence = getRecurringOccurrence<TResource, TEvent>(master, occurrenceStart)
        if (!occurrence) {
          return {
            success: false,
            error: {
        ...
        const effectiveStart =
          updates.start != null ? toPlainDateTimeString(updates.start) : occurrenceStartStr
        const effectiveEnd =
          updates.end != null
        ...
        const oldEndDate = Temporal.PlainDate.from(occurrenceEndStr.slice(0, 10)).add({ days: 1 })
        const newEndDate = Temporal.PlainDate.from(effectiveEnd.slice(0, 10)).add({ days: 1 })
        const rangeEndPlain =
        ...
        const normalizedUpdates = {
          ...
          ...(updates.end != null || updates.start != null ? { end: effectiveEnd } : {}),
        ...
        if (occurrenceStart === toPlainDateTimeString(master.start)) {
          const withoutOwnException =
            recurrenceUpdate != null
              ? null
              : dropOccurrenceExceptions<TResource, TEvent>(master.recurrence, occurrenceStart)
      ...
      removeRecurringEvent: (eventId, options) => {
        ...
        writeOccurrenceRemove(host, master, 'thisAndFollowing', occurrenceStart, {
          type: 'removed',
          eventId: occurrence.id,
          eventTitle: occurrence.title,
          start: occurrenceStartStr,
