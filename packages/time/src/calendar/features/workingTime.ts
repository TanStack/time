...
import type { MinuteRange, WorkingCalendar, WorkingTimeRange } from '~/workingTime'
...
export interface WorkingTimeApi<TResource extends Resource> {
  getEffectiveCalendar: (target?: WorkingTimeTarget<TResource>) => string | undefined
  getWorkingIntervals: (
    range: WorkingTimeRange,
    target?: WorkingTimeTarget<TResource>,
  ) => Array<WorkingTimeRange>
  getWorkingMinutes: (date: string, target?: WorkingTimeTarget<TResource>) => Array<MinuteRange>
  getNonWorkingMinutes: (date: string, target?: WorkingTimeTarget<TResource>) => Array<MinuteRange>
  isWorkingTime: (range: WorkingTimeRange, target?: WorkingTimeTarget<TResource>) => boolean
}

export function workingTimeFeature<
  TResource extends Resource,
  TEvent extends Event<TResource>,
>(): CalendarFeature<TResource, TEvent, object, WorkingTimeApi<TResource>, 'workingTime'> {
  ...
  const resourcesOf = (host: CalendarHost<TResource, TEvent>): Array<TResource> => {
    const { resources, workingTime } = host.getOptions()
    if (resources !== cachedForResources || workingTime.calendars !== cachedForCalendars) {
      minuteCache.clear()
  ...
  const targetResourceIds = (target?: WorkingTimeTarget<TResource>): Array<string> | undefined => {
  ...
  const layersOf = (
    ...
    const bases = ids
      ? resources
          ...
          .map((resource) => resource.calendarId ?? workingTime.defaultCalendarId)
  ...
  const workingMinutes = (
    ...
    const merged = mergeMinuteRanges(
      layers.flatMap((stack) => resolveLayeredDayMinutes(stack, date, workingTime.calendars)),
    )
  ...
  return {
    ...
    api: (host) => ({
      getEffectiveCalendar: (target) => {
        ...
        const resource =
          id != null ? resourcesOf(host).find((candidate) => candidate.id === id) : undefined
