...
const layoutOf = (event: TestEvent): EventLayout => (event as LaidOutEvent<TestEvent>)[LAYOUT_FIELD]
...
describe('layoutModule', () => {
  ...
  it('lays out all-day and timed events on separate tracks', () => {
    ...
    const layouts = new Map(kernel.project(viewport).map((e) => [e.id, layoutOf(e)]))
  ...
  it('skips splitting when splitMultiDayEvents is false', () => {
    ...
    }).use(layoutModule<TestEvent>({ timeZone: UTC, splitMultiDayEvents: false }))
