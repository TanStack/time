...
const makeKernel = () =>
  new Kernel<CalEvent>({ events: [master()] }).use(
    recurrenceModule<CalEvent>(options),
  );
...
describe("recurrenceModule materialize", () => {
  ...
  it('expands an edit intent with scope "thisAndFollowing" into a split pair', () => {
    ...
    const split = kernel
      .getEvents()
      .find((e) => e.id !== "m" && e.start === "2026-01-08T11:00:00");
  ...
  it('expands a remove intent with scope "this" into an exDate update', () => {
    ...
    expect(kernel.getEvent("m")!.recurrence!.exDates).toContain(
      "2026-01-08T09:00:00",
    );
  });
