...
export function filterModule<E extends KernelEvent>(
  options: FilterModuleOptions<E>,
): Module<E> {
  return {
    name: "filter",
    contributions: [
      {
        ...
        run: (ctx) => {
          ...
          return ctx.events.filter((event) =>
            predicates.every((predicate) => predicate(event)),
          );
        },
      },
    ],
