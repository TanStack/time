...
import type { Event, EventDateTimeInput, RecurrenceEditScope } from '~/calendar/types'
...
export function removeOccurrenceIntent(payload: RemoveOccurrencePayload): IntentOp {
...
function resolveEdit<E extends KernelEvent>(
  ...
  const occurrenceStart = resolveOccurrenceStart(master.start, payload.occurrenceStart)
  ...
  const effectiveStart = nextStart != null ? toPlainDateTimeString(nextStart) : occurrenceStartStr
  const effectiveEnd =
    nextEnd != null
  ...
  const normalizedUpdates = {
    ...
    ...(recurrenceUpdate != null ? { recurrence: normalizeRecurrenceRule(recurrenceUpdate) } : {}),
  }
...
export function recurrenceModule<E extends KernelEvent>(
  ...
  return {
    ...
    getRequiredRange: (op) => {
      ...
      if (op.intent !== EDIT_OCCURRENCE_INTENT && op.intent !== REMOVE_OCCURRENCE_INTENT) {
        return null
      }
      ...
      },
      {
        ...
        run: (batch, ctx) => {
          ...
          for (const op of batch.ops) {
            ...
            const payload = op.payload as EditOccurrencePayload | RemoveOccurrencePayload
            const master = ctx.getEvent(payload.masterId) as (Event & E) | undefined
            if (!master) continue

            if (op.intent === EDIT_OCCURRENCE_INTENT) {
              ops.push(...resolveEdit<E>(payload as EditOccurrencePayload, master, isTaken))
            } else {
              ops.push(...resolveRemove<E>(payload as RemoveOccurrencePayload, master))
            }
          }
