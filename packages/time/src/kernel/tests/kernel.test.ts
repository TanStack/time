...
const evt = (id: string, start: string, end: string, title = id): TestEvent => ({
  id,
  start,
  end,
  title,
...
describe('Kernel', () => {
  ...
  describe('write pipeline', () => {
    ...
    it('commits several ops as one batch', () => {
      ...
      expect(result.status).toBe('committed')
      expect(result.status === 'committed' && result.batch.reason).toBe('split')
      expect(kernel.getEvents().map((e) => e.id)).toEqual(['b'])
    })
        ...
        contributions: [
          {
            ...
            run: (batch) => ({
              ...
              ops: [
                ...
                {
                  kind: 'add',
                  event: evt('shadow', '2026-01-01T11:00:00Z', '2026-01-01T12:00:00Z'),
                },
              ],
            }),
          },
  ...
  describe('module requirements (ADR 0009)', () => {
    ...
    it('is satisfied whichever order the modules are given in', () => {
      ...
      expect(() => createKernel({ modules: { resize, availability } })).not.toThrow()
      expect(() => createKernel({ modules: { availability, resize } })).not.toThrow()
    })
  })
})
