...
const hoursFromFirstDay = (value: string | Date | number, firstDay: Temporal.PlainDate): number => {
...
export function layoutTimelineRange<E extends TimelineSpanEvent>(
  ...
  input.events.forEach((event, index) => {
    ...
    let lane = lanes.findIndex(
      (placed) => !placed.some((item) => startFraction < item.end && endFraction > item.start),
    )
...
export function currentTimeFraction(input: {
  ...
  return (dayIndex * HOURS_IN_DAY + hourFraction) / (input.isoDates.length * HOURS_IN_DAY)
}
