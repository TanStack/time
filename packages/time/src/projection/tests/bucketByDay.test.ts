...
describe("splitMultiDay", () => {
  it("leaves a single-day event untouched", () => {
    expect(
      spansMultipleDays(evt("a", "2026-01-05T09:00:00", "2026-01-05T10:00:00")),
    ).toBe(false);
  });

  it("detects a multi-day span", () => {
    expect(
      spansMultipleDays(evt("a", "2026-01-05T09:00:00", "2026-01-07T10:00:00")),
    ).toBe(true);
  });

  it("splits a 3-day event into one segment per day", () => {
    const segments = splitMultiDay(
      evt("a", "2026-01-05T09:00:00", "2026-01-07T10:00:00"),
      UTC,
    );
  ...
  it("tags every segment with the original span", () => {
    const segments = splitMultiDay(
      evt("a", "2026-01-05T09:00:00", "2026-01-06T10:00:00"),
      UTC,
    );
...
describe("bucketByDay", () => {
  ...
  it("places each segment of a multi-day event in its own day", () => {
    const buckets = bucketByDay(
      [evt("a", "2026-01-05T22:00:00", "2026-01-07T02:00:00")],
      UTC,
    );
