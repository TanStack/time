...
export function bucketByDay<E extends SplittableEvent>(
  ...
  for (const event of splitEventsByDay(events, timeZone)) {
    const dateKey = Temporal.PlainDateTime.from(toPlainDateTimeString(event.start))
      .toPlainDate()
...
export function buildDays<E extends SplittableEvent>(input: BuildDaysInput<E>): Array<DayView<E>> {
  const buckets = bucketByDay(input.events, input.timeZone)
  const today = input.today ?? Temporal.Now.plainDateISO().toString({ calendarName: 'never' })
