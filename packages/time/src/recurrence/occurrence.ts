...
>(
  ...
  const first = occurrences.find((occurrence) => isoDateOf(occurrence.start) >= searchStart)
...
>(master: TEvent, fromIsoDate: string): string | null {
  ...
  const earlier = expandRecurringEvent<TResource, TEvent>(master, masterStart, fromIsoDate)
