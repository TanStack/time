...
describe('recurrence helpers', () => {
  ...
  it('resolveOccurrenceStart falls back to the master start', () => {
    expect(resolveOccurrenceStart('2026-01-05T09:00:00')).toBe('2026-01-05T09:00:00')
    expect(resolveOccurrenceStart('2026-01-05T09:00:00', '2026-01-08T09:00:00')).toBe(
      '2026-01-08T09:00:00',
    )
  })

  it('recurrenceInputMatchesOccurrence matches by date or datetime', () => {
    expect(recurrenceInputMatchesOccurrence('2026-01-06', '2026-01-06T09:00:00')).toBe(true)
    expect(recurrenceInputMatchesOccurrence('2026-01-06T09:00:00', '2026-01-06T09:00:00')).toBe(
      true,
    )
    expect(recurrenceInputMatchesOccurrence('2026-01-07', '2026-01-06T09:00:00')).toBe(false)
  })

  it('compareRecurrenceInputToOccurrence orders inputs', () => {
    expect(compareRecurrenceInputToOccurrence('2026-01-07', '2026-01-06T09:00:00')).toBeGreaterThan(
      0,
    )
  })
  ...
  it('durationPreservingEnd keeps the duration when the start moves', () => {
    expect(
      durationPreservingEnd('2026-01-05T09:00:00', '2026-01-05T10:30:00', '2026-01-06T14:00:00'),
    ).toBe('2026-01-06T15:30:00')
  })

  it('makeSplitRecurringEventId avoids collisions', () => {
    const taken = new Set(['m_20260108T090000'])
    expect(makeSplitRecurringEventId('m', '2026-01-08T09:00:00', (id) => taken.has(id))).toBe(
      'm_20260108T090000_1',
    )
  })
...
describe('materializeRecurringEdit', () => {
  it('scope "this" adds an override without a split event', () => {
    ...
    const { nextMaster, addedEvents }: MaterializeResult<Event> = materializeRecurringEdit(input)
