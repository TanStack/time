...
import type { Event, EventDateTimeInput, RecurrenceRule, Resource } from '~/calendar/types'
...
export function normalizeRecurrenceRule<TResource extends Resource>(
  ...
  return {
    ...
    exDates: rule.exDates?.map((value) =>
      typeof value === 'string' && !value.includes('T') ? value : toPlainDateTimeString(value),
    ),
    overrides: rule.overrides?.map((override) => ({
      ...override,
      originalStart:
        typeof override.originalStart === 'string' && !override.originalStart.includes('T')
          ? override.originalStart
          : toPlainDateTimeString(override.originalStart),
      ...(override.start != null ? { start: toPlainDateTimeString(override.start) } : {}),
      ...(override.end != null ? { end: toPlainDateTimeString(override.end) } : {}),
    })),
  }
}
...
>(master: TEvent, occurrenceStart: string): TEvent | null {
  ...
  const occurrences = expandRecurringEvent<TResource, TEvent>(master, windowStart, windowEnd)
  return (
    occurrences.find((occ) => (occ._occurrenceOriginalStart ?? occ.start) === occurrenceStart) ??
    null
  )
}
...
>(recurrence: TEvent['recurrence'], occurrenceStart: string): TEvent['recurrence'] | null {
  ...
  const overrides = (rule.overrides ?? []).filter(
    (override) => !recurrenceInputMatchesOccurrence(override.originalStart, occurrenceStart),
  )
...
export interface MaterializeEditInput<TResource extends Resource, TEvent extends Event<TResource>> {
  master: TEvent
...
>(input: MaterializeEditInput<TResource, TEvent>): MaterializeResult<TEvent> {
  ...
  const remainingRule = normalizeRecurrenceRule({
    ...
    overrides: rule.overrides?.filter(
      (override) => compareRecurrenceInputToOccurrence(override.originalStart, occurrenceStart) > 0,
    ),
  })
  ...
  const splitEvent = {
    ...
    recurrence:
      recurrenceUpdate != null ? normalizeRecurrenceRule(recurrenceUpdate) : remainingRule,
