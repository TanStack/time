...
>(event: TEvent, windowStart: string, windowEnd: string): Array<TEvent> {
  ...
  for (const exDate of rule.exDates ?? []) {
    if (typeof exDate === 'string' && !exDate.includes('T')) {
      exDates.add(Temporal.PlainDate.from(exDate).toString({ calendarName: 'never' }))
    } else {
      const normalized = toPlainDateTimeString(exDate)
      exDateTimes.add(normalized)
      exDates.add(
        Temporal.PlainDateTime.from(normalized).toPlainDate().toString({ calendarName: 'never' }),
      )
    }
  }
  ...
  const masterMonday = masterStart.toPlainDate().subtract({ days: masterStart.dayOfWeek - 1 })
  ...
  while (step <= MAX_STEPS) {
    ...
    const uniqueCandidates = Array.from(
      new Map(
        candidates
          .filter((candidate) => Temporal.PlainDateTime.compare(candidate, masterStart) >= 0)
    ...
    for (const candidateStart of uniqueCandidates) {
      ...
      const excluded = exDateTimes.has(originalStartStr) || exDates.has(candidateDateStr)

      if (!excluded) {
        const generatedId = occurrenceIndex === 0 ? event.id : `${event.id}_${occurrenceIndex}`
        const overrideStart = override?.start
        const overrideEnd = override?.end
        const occurrenceStartStr =
          overrideStart != null ? toPlainDateTimeString(overrideStart) : originalStartStr
        ...
        const occurrenceStartDate = Temporal.PlainDateTime.from(occurrenceStartStr).toPlainDate()
        ...
        const endsAfterWindowStart =
          Temporal.PlainDate.compare(occurrenceStartDate, windowStartDate) >= 0 ||
          Temporal.PlainDateTime.compare(occurrenceEnd, windowStartDate.toPlainDateTime()) > 0

        if (startsBeforeWindowEnd && endsAfterWindowStart) {
          const overrideFields = override ? ({ ...override } as Record<string, unknown>) : {}
...
function addSafeMonths(dt: Temporal.PlainDateTime, months: number): Temporal.PlainDateTime {
  let year = dt.year
