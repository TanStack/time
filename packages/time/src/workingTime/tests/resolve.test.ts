...
describe('resolveCalendarChain', () => {
  ...
  it('stops at a missing parent instead of failing', () => {
    ...
    expect(resolveCalendarChain('ana', [orphan]).map((c) => c.id)).toEqual(['ana'])
  })
  ...
  it('throws on a parent cycle', () => {
    ...
    expect(() => resolveCalendarChain('a', calendars)).toThrow(/Working calendar cycle/)
  })
})
...
describe('resolveLayeredDayMinutes', () => {
  ...
  it('layers an event calendar over the resource chain', () => {
    expect(resolveLayeredDayMinutes(['project', 'late-meeting'], MONDAY, calendars)).toEqual([
      { startMinutes: 540, endMinutes: 1020 },
  ...
  it('gives the event layer the last word at equal specificity', () => {
    expect(resolveLayeredDayMinutes(['closed-monday', 'late-meeting'], MONDAY, calendars)).toEqual([
      { startMinutes: 1080, endMinutes: 1200 },
    ])
  })

  it('ignores layers that resolve to nothing', () => {
    expect(resolveLayeredDayMinutes([undefined, 'project'], MONDAY, calendars)).toEqual([
      { startMinutes: 540, endMinutes: 1020 },
    ])
...
describe('getWorkingTime', () => {
  it('clips the first and last day to the range', () => {
    expect(
      getWorkingTime('project', { start: `${MONDAY}T12:00`, end: '2026-01-07T10:00' }, [
        officeWeek,
      ]),
    ).toEqual([
  ...
  it('treats the range end as exclusive', () => {
    expect(getWorkingTime('project', { start: MONDAY, end: TUESDAY }, [officeWeek])).toEqual([
      { start: '2026-01-05T09:00:00', end: '2026-01-05T17:00:00' },
    ])
  })
  ...
  it('merges across midnight when the calendar never closes', () => {
    ...
    expect(getWorkingTime('24-7', { start: MONDAY, end: '2026-01-07' }, [alwaysOpen])).toEqual([
      { start: '2026-01-05T00:00:00', end: '2026-01-07T00:00:00' },
    ])
  })

  it('returns nothing for an empty or inverted range', () => {
    expect(getWorkingTime('project', { start: MONDAY, end: MONDAY }, [officeWeek])).toEqual([])
    expect(getWorkingTime('project', { start: TUESDAY, end: MONDAY }, [officeWeek])).toEqual([])
    expect(getWorkingTime('nope', { start: MONDAY, end: SATURDAY }, [])).toEqual([])
  })
})
