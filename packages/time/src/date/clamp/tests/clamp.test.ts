...
describe('clamp', () => {
  ...
  describe('with different input types', () => {
    ...
    test('should work with ZonedDateTime', () => {
      const zdt = Temporal.ZonedDateTime.from('2024-02-15T00:00:00Z[UTC][u-ca=gregory]')
      const result = clamp(zdt, {
        range: {
          start: Temporal.ZonedDateTime.from('2024-03-01T00:00:00Z[UTC][u-ca=gregory]'),
          end: Temporal.ZonedDateTime.from('2024-03-31T00:00:00Z[UTC][u-ca=gregory]'),
        },
        timeZone: 'UTC',
      })
      expect(result.toISOString()).toBe('2024-03-01T00:00:00.000Z')
