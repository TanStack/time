...
describe('count', () => {
  ...
  describe('with different input types', () => {
    test('should work with Date objects', () => {
      const result = count(new Date('2024-03-01T00:00:00Z'), new Date('2024-03-31T00:00:00Z'), {
        unit: 'day',
        timeZone: 'UTC',
      })
      expect(result).toBe(30)
    ...
    test('should work with ZonedDateTime', () => {
      const start = Temporal.ZonedDateTime.from('2024-03-01T00:00:00Z[UTC][u-ca=gregory]')
      const end = Temporal.ZonedDateTime.from('2024-03-31T00:00:00Z[UTC][u-ca=gregory]')
      const result = count(start, end, { unit: 'day', timeZone: 'UTC' })
      expect(result).toBe(30)
    })
  })
