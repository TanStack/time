...
export function resolveCalendarId(calendar: Temporal.CalendarLike): string {
  return Temporal.PlainDate.from(REFERENCE_ISO_DATE).withCalendar(calendar)
    .calendarId;
}
...
export function toCalendarDate(
  ...
  return (
    ...
    cacheSet(
      plainDateCache,
      key,
      Temporal.PlainDate.from(isoDate).withCalendar(calendarId),
    )
  );
}
...
export function getDateParts(
  ...
  return (
    partsCache.get(key) ??
    cacheSet(partsCache, key, buildParts(isoDate, calendarId, locale))
  );
}
