...
describe('getDateParts', () => {
  ...
  test("takes the day token from the calendar's own numbering", () => {
    expect(getDateParts('2026-08-21', { calendar: 'chinese', locale: 'zh-CN' })).toMatchObject({
      monthLong: '\u4e03\u6708',
      dayOfMonth: '\u521d\u4e5d',
    })
    expect(getDateParts('2026-08-21', { calendar: 'hebrew', locale: 'he-IL' }).dayOfMonth).toBe(
      '\u05d7\u05f3',
    )
    expect(getDateParts('2026-08-21', { calendar: 'gregory', locale: 'en-US' }).dayOfMonth).toBe(
      '21',
    )
  })
  ...
  test('expands a year-month string to its first day', () => {
    expect(getDateParts('2025-07', { calendar: 'gregory', locale: 'en-US' }).day).toBe(1)
  })
  ...
  test('labels lunisolar dates on runtimes whose ICU breaks Temporal', () => {
    ...
    Intl.DateTimeFormat = function patched(locale?: string, options?: Intl.DateTimeFormatOptions) {
      ...
      return {
        ...
        formatToParts: (date?: Date) =>
          formatter
            .formatToParts(date)
            .map((part) => (part.type === 'month' ? { ...part, value: `Mo${part.value}` } : part)),
    ...
    expect(`${parts.monthLong}${parts.dayOfMonth}`).toBe('\u4e8c\u6708\u521d\u4e03')
  })
  ...
  test('returns a cached instance per locale and calendar', () => {
    ...
    expect(getDateParts(ISO_DATE, { calendar: 'gregory', locale: 'en-US' })).toBe(first)
    expect(getDateParts(ISO_DATE, { calendar: 'gregory', locale: 'de-DE' })).not.toBe(first)
    expect(getDateParts(ISO_DATE, { calendar: 'chinese', locale: 'en-US' })).not.toBe(first)
  })
})
