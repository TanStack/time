...
describe('range', () => {
  ...
  describe('with different input types', () => {
    ...
    test('should work with ZonedDateTime', () => {
      const startZdt = Temporal.ZonedDateTime.from('2024-03-01T00:00:00Z[UTC][u-ca=gregory]')
      const endZdt = Temporal.ZonedDateTime.from('2024-03-03T00:00:00Z[UTC][u-ca=gregory]')
  ...
  describe('output', () => {
    test('should return an array of Dates', () => {
      ...
      expect(result[0]!.toISOString()).toBe('2024-03-01T00:00:00.000Z')
      expect(result[0]!.getTime()).toBe(new Date('2024-03-01T00:00:00Z').getTime())
    })
  })
