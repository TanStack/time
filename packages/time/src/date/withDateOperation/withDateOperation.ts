...
export function resolveOptions(options: DateOperationOptions): ResolvedDateOperationOptions {
...
export function withDateOperation<TArgs>(
  ...
  return (input: DateInput, options: DateOperationOptions & TArgs): Date => {
    ...
    const inputZdt = toZonedDateTime(input, resolved.timeZone, resolved.calendar)
