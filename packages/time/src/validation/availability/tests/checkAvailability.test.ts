...
const hours = (id: string, ...slots: Array<RecurrentWorkingInterval>): WorkingCalendar => ({
  id,
...
describe('checkAvailability', () => {
  ...
  it('lets an event calendar open time the resource calendar closes', () => {
    ...
    expect(checkAvailability({ event: late, resources: [nineToFive], workingTime })).toHaveLength(1)
  ...
  it('blocks when any assigned resource is closed, by default', () => {
    const conflicts = checkAvailability({
      ...
      resources: [nineToFive, { id: 'r9', label: 'Always', calendarId: 'open' }],
      workingTime,
    })
  ...
  it('blocks under the union policy only when every resource is closed', () => {
    ...
    expect(checkAvailability({ event, resources: [nineToFive], workingTime: union })).toHaveLength(
      1,
    )
  })
