...
describe('mergeUnavailableMinuteRanges', () => {
  it('returns null when no resource constrains the day', () => {
    ...
    expect(mergeUnavailableMinuteRanges(undefined, MONDAY, workingTime)).toBeNull()
  })

  it('returns null when resourceIds match nothing', () => {
    expect(mergeUnavailableMinuteRanges([office], MONDAY, workingTime, ['other'])).toBeNull()
  })
  ...
  it('blocks the whole day when the weekday has no slot', () => {
    expect(mergeUnavailableMinuteRanges([office], SATURDAY, workingTime)).toEqual([
      { startMinutes: 0, endMinutes: 1440 },
    ])
  })

  it('blocks the whole day when no resource references a calendar', () => {
    expect(mergeUnavailableMinuteRanges([resource('free')], MONDAY, workingTime)).toEqual([
      { startMinutes: 0, endMinutes: 1440 },
    ])
  })
  ...
  it('unions working time across resources', () => {
    ...
    expect(mergeUnavailableMinuteRanges([office, evening], MONDAY, workingTime)).toEqual([
      { startMinutes: 0, endMinutes: 540 },
  ...
  it('merges touching windows into one gap-free block', () => {
    ...
    expect(mergeUnavailableMinuteRanges([office, late], MONDAY, workingTime)).toEqual([
      { startMinutes: 0, endMinutes: 540 },
  ...
  it('narrows the union to the requested resources', () => {
    ...
    expect(
      mergeUnavailableMinuteRanges([office, evening], MONDAY, workingTime, ['evening-only']),
    ).toEqual([
  ...
  it('does not leak mutable state between calls', () => {
    ...
    expect(mergeUnavailableMinuteRanges([office], MONDAY, workingTime)![0]).toEqual({
      startMinutes: 0,
      endMinutes: 540,
    })
  })
