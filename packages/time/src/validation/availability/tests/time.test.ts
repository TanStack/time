...
describe('time helpers', () => {
  ...
  it('resourceDayWorkingTime merges overlapping intervals and derives gaps', () => {
    ...
    expect(info.working).toEqual<Array<MinuteRange>>([{ startMinutes: 540, endMinutes: 1020 }])
  ...
  it('resourceDayWorkingTime separates a closed calendar from a missing one', () => {
    ...
    expect(resourceDayWorkingTime({ calendarId: 'closed' }, MONDAY, workingTime).configured).toBe(
      true,
    )
  })
  ...
  it("effectiveCalendarId prefers the resource's own calendar", () => {
    ...
    expect(effectiveCalendarId({}, { ...workingTime, defaultCalendarId: 'closed' })).toBe('closed')
