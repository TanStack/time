...
import type { AvailabilityConflict, AvailabilityResourceInput } from './checkAvailability'
...
export function checkDaySpan(input: CheckDaySpanInput): Array<AvailabilityConflict> {
  ...
  const unavailable =
    mergeUnavailableMinuteRanges(resources, date, workingTime, undefined, input.eventCalendarId) ??
    []
  ...
  for (const resource of resources) {
    ...
    const usedByOthers = otherEvents
      .filter((event) => event.resourceIds.includes(resource.id) && overlaps(event, span))
