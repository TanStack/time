...
describe('workingMinutesBetween', () => {
  ...
  it('counts only working minutes inside the span', () => {
    expect(
      workingMinutesBetween({ start: `${MON}T08:00:00`, end: `${MON}T12:00:00` }, [], withCalendar),
    ).toBe(180)
  })

  it('skips the non-working gap between two days', () => {
    expect(
      workingMinutesBetween({ start: `${MON}T16:00:00`, end: `${TUE}T10:00:00` }, [], withCalendar),
    ).toBe(120)
  })

  it('counts a span that lands entirely outside working time as zero', () => {
    expect(
      workingMinutesBetween({ start: `${SAT}T09:00:00`, end: `${SAT}T17:00:00` }, [], withCalendar),
    ).toBe(0)
  })

  it('returns zero for an inverted span', () => {
    expect(
      workingMinutesBetween({ start: `${MON}T11:00:00`, end: `${MON}T09:00:00` }, [], withCalendar),
    ).toBe(0)
  })
...
describe('checkDuration', () => {
  it('stays silent when neither field is declared', () => {
    expect(checkDuration({ event: event(), workingTime: withCalendar })).toEqual([])
  })

  it("accepts a duration that matches the span's working minutes", () => {
    expect(checkDuration({ event: event(180), workingTime: withCalendar })).toEqual([])
  })
