...
describe("checkConstraint", () => {
  ...
  it("compares a date-only constraint by civil day", () => {
    expect(
      checkConstraint(event(START, END, "start-no-earlier-than", "2026-03-02")),
    ).toBeNull();
    expect(
      checkConstraint(event(START, END, "must-start-on", "2026-03-02")),
    ).toBeNull();
    expect(
      checkConstraint(event(START, END, "must-start-on", "2026-03-03")),
    ).not.toBeNull();
  });
  ...
  it("compares a datetime constraint by the second", () => {
    expect(
      checkConstraint(
        event(START, END, "start-no-earlier-than", "2026-03-02T09:00:01"),
      ),
    ).not.toBeNull();
  });

  it("anchors a finish constraint on the end", () => {
    const conflict = checkConstraint(
      event(START, END, "finish-no-later-than", "2026-03-01"),
    );
  ...
  it("names the violated rule in the message", () => {
    expect(
      checkConstraint(event(START, END, "must-start-on", "2026-03-05"))
        ?.message,
    ).toBe('"A" must start on 2026-03-05 (must-start-on)');
  });
});
