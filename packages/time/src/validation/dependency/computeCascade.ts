...
export function computeCascade(input: CascadeInput): Array<CascadeShift> {
  ...
  const srcStartMs =
    Temporal.PlainDateTime.from(sourceEvent.start).toZonedDateTime(timeZone)
      .epochMilliseconds + deltaMs;
  const srcEndMs =
    Temporal.PlainDateTime.from(sourceEvent.end).toZonedDateTime(timeZone)
      .epochMilliseconds + deltaMs;
  ...
  while (queue.length > 0) {
    ...
    for (const sId of successorIds) {
      ...
      const sEndMs = Temporal.PlainDateTime.from(s.end).toZonedDateTime(
        timeZone,
      ).epochMilliseconds;
