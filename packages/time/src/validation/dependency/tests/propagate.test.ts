...
import { propagateToDependents, propagateToPredecessors, shiftToSatisfyLink } from '../propagate'
...
describe('propagateToDependents', () => {
  ...
  it('stops where the chain still has slack', () => {
    ...
    expect(shifts).toEqual([{ id: 'b', newStart: at('10:30'), newEnd: at('11:30') }])
  })
  ...
  it('honours each link type', () => {
    const shiftFor = (type: DependencyType, start: string, end: string) =>
      propagateToDependents({
        sourceId: 'a',
        timeZone: UTC,
        events: [event('a', '09:00', '12:00'), event('b', start, end, [{ id: 'a', type }])],
      })[0]
  ...
  it('returns nothing for an unknown or leaf source', () => {
    ...
    expect(propagateToDependents({ sourceId: 'a', timeZone: UTC, events })).toEqual([])
    expect(propagateToDependents({ sourceId: 'nope', timeZone: UTC, events })).toEqual([])
  })
})
...
describe('propagateToPredecessors', () => {
  it('pulls a predecessor back when the successor moves earlier', () => {
    ...
    expect(shifts).toEqual([{ id: 'p', newStart: at('08:00'), newEnd: at('09:00') }])
  })
...
describe('dependency lag', () => {
  ...
  it('pulls a predecessor back by the lag as well', () => {
    ...
    expect(shifts).toEqual([{ id: 'a', newStart: at('08:30'), newEnd: at('09:30') }])
  })
